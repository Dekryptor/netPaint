...
(function() {
  ...
  var xdraw: CustomHTMLElement = Object.create(HTMLElement.prototype);
      
  //LifecycleCallbacks Registrieren
         
  xdraw.attachedCallback = function() {
    var that: CustomHTMLElement = this;
    ...
    function doDraw(event: MouseEvent |TouchEvent) {
  ...
  xdraw.createdCallback = function() {
    ...
    var renderState: number = 0;
    var imageBuffer: ImageData;
    this.drawCandidate = new PaintOperation();
    var canvas = this.shadowDOM.querySelector("canvas");
    var ctx: CanvasRenderingContext2D = canvas.getContext("2d");
    var candidate: PaintOperation = this.drawCandidate;
    var paintstack: Array<PaintOperation> = this.paintstack;
    ...
    this.render = function() {
      if (renderState != paintstack.length){
          if(renderState > paintstack.length){
            ...
            //Element wurde gelöscht, gesamten canvas neu malen
